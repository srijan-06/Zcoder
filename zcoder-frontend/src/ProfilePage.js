...
const ProfilePage = () => {
    ...
    const [email, setEmail] = useState(currentUser!==null ? currentUser.email:'');
    ...
    const regionSubmit = async (e) => {
        e.preventDefault();
        
        try {
            // Call the backend signup API
            const signupResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}/api/auth/signup`, {
                method: 'POST',
            ...
            const updateResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}/user/${signupData._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    bio: '',
            ...
            history.push(`/profile/${signupData._id}`);
            console.log('Form submitted:', { country, region, city, institute, ...handles });
            setSignupStep('null');
            setNewProfile(false);
